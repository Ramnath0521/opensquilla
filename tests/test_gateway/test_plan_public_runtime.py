...
async def test_plan_revision_result_is_durable_and_attributed_to_its_turn(
    ...
    async with _open_plan_rpc_stack(tmp_path / "outcomes.sqlite", handler=handler) as stack:
        ...
        async def settle(task_id, **fields):
            nonlocal attempts
            attempts += 1
            if attempts <= failed_writes:
                raise RuntimeError("synthetic terminal write failure")
            # A concurrent metadata writer must survive both direct settlement
            # and a retry built from the earlier admission snapshot.
            latest = await stack.storage.get_agent_task(task_id)
            details = dict(latest.details)
            details["metadata"] = {
                **details["metadata"], "synthetic_audit": {"revision": attempts},
            }
            await stack.storage.update_agent_task(task_id, details=details)
            return await original_settle(task_id, **fields)

        monkeypatch.setattr(stack.storage, "settle_agent_task", settle)
        ...
        if stack.runtime._terminal_retry_task is not None:
        ...
        assert attempts == failed_writes + 1
        assert stored.details["metadata"]["synthetic_audit"] == {"revision": attempts}
